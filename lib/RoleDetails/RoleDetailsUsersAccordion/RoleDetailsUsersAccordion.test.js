...
describe('RoleDetailsUsersAccordion', () => {
  ...
  it('renders user full name as plain text when hideUserLink is true', () => {
    useUsersByRoleId.mockReturnValue({ users, isLoading: false });
    useUserGroups.mockReturnValue({ userGroups, isLoading: false });
    ...
    const { getByText } = render(
      renderWithRouter(
        <QueryClientProvider client={queryClient}>
          <RoleDetailsUsersAccordion roleId="1" hideUserLink />
        </QueryClientProvider>
      )
    );

    const text = getByText(users[0].personal.firstName, { exact: false });
    expect(text.tagName).not.toBe('A');
  });

  it('renders user full name as a link when hideUserLink is false', () => {
    useUsersByRoleId.mockReturnValue({ users, isLoading: false });
    useUserGroups.mockReturnValue({ userGroups, isLoading: false });
    ...
    const link = getByText(users[0].personal.firstName, { exact: false });
