...
export const RoleDetailsUsersAccordion = ({ roleId, tenantId }) => {
  ...
  const [noneKeycloakUsersList, setNoneKeycloakUsersList] = useState([]);
  ...
  const onSubmitCreateKeycloakAuthUser = async () => {
    ...
    for (const userId of noneKeycloakUsersList) {
    ...
    <Accordion
      ...
      displayWhenOpen={(
        <RoleDetailsAssignUsers
          setNoneKeycloakUsersList={setNoneKeycloakUsersList}
    ...
    >
      ...
      <ConfirmationModal
        ...
        <FormattedMessage
          ...
          values={{ users: noneKeycloakUsersList.join(', ') }}
        />
        }
