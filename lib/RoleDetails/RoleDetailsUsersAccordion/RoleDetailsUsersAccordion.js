...
import { SETTINGS_AUTHORIZATION_ROLES_API } from '../../constants';
import {
  useUserGroups,
...
export const RoleDetailsUsersAccordion = ({ path, roleId, tenantId }) => {
  ...
  const usersData = sortedUsers.map(i => {
    const fullName = path === `/${SETTINGS_AUTHORIZATION_ROLES_API}` ? (<LinkedUser user={i} />) : getFullName(i);
