 ...
 *
 * @param {*} previousUserIds previous members
 * @param {*} currentUserIds current members
 * @param {*} roleId ID of role whose membership is changing
 * @param {*} queryClient react-query.queryClient
 * @param {*} ky okapiKy instance
 * @returns
 */
export async function createUserRolesRequests(previousUserIds, currentUserIds, roleId, queryClient, ky) {
  const requests = [];
...
/**
 * Asynchronously checks the authentication of Keycloak users based on the provided user IDs.
 *
 * @param {Object} options - The options object.
 * @param {Object} options.roleId - The role id.
 * @param {Array} options.userIds - An array of user IDs to check authentication for.
 * @param {Object} options.queryClient - The query client object for fetching data.
 * @param {Object} options.ky - The ky instance for making HTTP requests.
 */

export async function getNonKeycloakUsers({ roleId, userIds, queryClient, ky }) {
  const nonKeycloakUsersList = [];

  const checkByUserId = async (userId) => {
    try {
      await queryClient.fetchQuery(
        `jit-auth-role-${roleId}-user-${userId}`,
        () => ky.get(`users-keycloak/auth-users/${userId}`),
      );
    } catch (error) {
      if (error?.response?.status === 404) {
        nonKeycloakUsersList.push(userId);
      }
    }
  };

  for (const userId of userIds) {
    await checkByUserId(userId);
  }

  return nonKeycloakUsersList;
}
