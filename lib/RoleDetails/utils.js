...
export async function checkAndGetNoneKeycloakAuthUsers({ roleId, userIds, queryClient, ky }) {
  const noneKeycloakUsersList = [];
  ...
  const checkByUserId = async (userId) => {
    try {
      ...
      if (error?.response?.status === 404) {
        noneKeycloakUsersList.push(userId);
      }
    }
  };
