...
export const RoleDetails = ({
  ...
  const [shouldConfirmAssignedRoleDeletion, setShouldConfirmAssignedRoleDeletion] = useState(false);
  ...
  const { users, isLoading: usersIsLoading, refetch: refetchAssignedUsers } = useUsersByRoleId(roleId, { tenantId });
  ...
  const checkIfRoleAssignedToCurrentUser = useCallback(() => {
    setIsDeleting(false); // close previous confirmation modal;
    if (users.some((u) => u.id === stripes.user.user.id)) {
      // display additional confirmation if the role-to-be-deleted belongs to the current user.
      setShouldConfirmAssignedRoleDeletion(true);
    } else {
      onRoleDelete();
    }
  }, [users, stripes?.user?.user?.id]);
  ...
  return (
    ...
    >
      ...
      <SessionConfirmationModal
        ...
        onConfirm={checkIfRoleAssignedToCurrentUser}
        onCancel={() => setIsDeleting(false)}
        confirmLabel={<FormattedMessage id="stripes-authorization-components.crud.delete" />}
        isConfirmButtonDisabled={isRoleDeleting || isSharedRoleDeleting || usersIsLoading}
        sessionKey="suppress-crud-delete-role-confirmation"
      />

      <SessionConfirmationModal
        open={shouldConfirmAssignedRoleDeletion}
        heading={<FormattedMessage id="stripes-authorization-components.crud.assignedRoleDeletion" />}
        message={(
          <FormattedMessage
            id="stripes-authorization-components.crud.deleteAssignedRoleConfirmation"
            values={{ term: role?.name }}
          />
        )}
        onConfirm={onRoleDelete}
        confirmLabel={<FormattedMessage id="stripes-authorization-components.crud.confirm" />}
        onCancel={() => setShouldConfirmAssignedRoleDeletion(false)}
        isConfirmButtonDisabled={isRoleDeleting || isSharedRoleDeleting}
        sessionKey="suppress-crud-delete-assigned-role-confirmation"
      />

      <ConfirmationModal
        open={isConfirmShareModalOpen}
        heading={intl.formatMessage({ id: 'stripes-authorization-components.consortiumManager.modal.confirmShare.all.heading' })}
        message={(
          <FormattedMessage
            id="stripes-authorization-components.consortiumManager.modal.confirmShare.all.message"
            values={{ term: role?.name }}
          />
        )} s
