...
describe('RoleDetailsAssignUsers', () => {
  ...
  describe('displays AssignUsers', () => {
    ...
    it('doesn\'t render assign button when list not loaded', () => {
      getByText('ui-users.permissions.assignUsers.actions.assign.notAvailable');
    });
  });

  describe('without ui-authorization-roles.users.settings.manage role, hides AssignUsers button', () => {
    IfPermission.mockImplementation(({ perm, children }) => (perm !== 'ui-authorization-roles.users.settings.manage' ? children : null));

    it('doesn\'t render assign button when list not loaded', () => {
      expect(screen.queryByText(('stripes-authorization-components.assignUnassign'))).not.toBeInTheDocument();
    });
  });
