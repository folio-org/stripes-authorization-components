...
import { apiVerbs, checkAndGetNoneKeycloakAuthUsers, createUserRolesRequests } from '../utils';
...
export const RoleDetailsAssignUsers = ({
  ...
  const handleCheckKeycloakUsers = async (newSelectedUsers) => {
    ...
    const keycloakNoneExistingUsersList = await checkAndGetNoneKeycloakAuthUsers({
      roleId,
    ...
    if (!keycloakNoneExistingUsersList.length) {
      setIsAuthUsersKeycloakConfirmationOpen(false);
    }
    setKeycloakNoneExistingUsers(keycloakNoneExistingUsersList);
  ...
  const assignUsers = async (newSelectedUsers) => {
    ...
    await handleCheckKeycloakUsers();
  };
...
RoleDetailsAssignUsers.propTypes = {
  ...
  setKeycloakNoneExistingUsers: PropTypes.func,
