...
export const RoleDetailsAssignUsers = ({
  ...
  const handleCheckKeycloakUsers = async (newSelectedUsers) => {
    ...
    const noneKeycloakUsersList = await checkAndGetNoneKeycloakAuthUsers({
      roleId,
    ...
    if (!noneKeycloakUsersList.length) {
      setIsAuthUsersKeycloakConfirmationOpen(false);
    }
    setNoneKeycloakUsersList(noneKeycloakUsersList);
...
RoleDetailsAssignUsers.propTypes = {
  ...
  setNoneKeycloakUsersList: PropTypes.func,
