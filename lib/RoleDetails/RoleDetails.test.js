...
describe('RoleDetails component', () => {
  ...
  describe('renders roles details pane with expanded information', () => {
    ...
    it('test confirm delete action', async () => {
      ...
      await userEvent.click(
        within(getByRole('dialog'))
          .getByRole('button', { name: 'stripes-authorization-components.crud.delete' })
      );
    ...
    it('test cancel delete action', async () => {
      ...
      await userEvent.click(
        within(getByRole('dialog'))
          .getByRole('button', { name: 'stripes-components.cancel' })
      );
    });
    ...
    describe('when the role being deleted is assigned to the current user', () => {
      // matches stripes.user.user.id from the mocked stripes object
      const currentUserId = 'b1add99d-530b-5912-94f3-4091b4d87e2c';

      beforeEach(() => {
        mockMutateDeleteRole.mockClear();
        useUsersByRoleId.mockReturnValue({ users: [{ id: currentUserId }], isLoading: false, refetch: jest.fn() });
      });

      afterEach(() => {
        useUsersByRoleId.mockReturnValue({ users: [], isLoading: false, refetch: jest.fn() });
      });

      it('shows an additional warning modal instead of deleting the role right away', async () => {
        const { getByRole } = renderComponent();
        ...
        expect(mockMutateDeleteRole).not.toHaveBeenCalled();
        expect(getByRole('dialog')).toHaveTextContent('stripes-authorization-components.crud.assignedRoleDeletion');
      });

      it('deletes the role once the warning modal is confirmed', async () => {
        const { getByRole } = renderComponent();
        ...
        await userEvent.click(
          within(getByRole('dialog'))
            .getByRole('button', { name: 'stripes-authorization-components.crud.confirm' })
        );

        expect(mockMutateDeleteRole).toHaveBeenCalledWith('2efe1d13-eff9-4b01-a2fe-512e9d5239c7');
      });

      it('does not delete the role when the warning modal is cancelled', async () => {
        const { getByRole } = renderComponent();
  ...
  describe('ECS mode', () => {
    ...
    it('should handle policy sharing when "Share to all" action is performed', async () => {
      ...
      await userEvent.click(
        within(screen.getByText('stripes-authorization-components.consortiumManager.modal.confirmShare.all.heading'))
