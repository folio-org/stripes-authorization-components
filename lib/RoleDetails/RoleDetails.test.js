...
describe('RoleDetails component', () => {
  ...
  describe('ECS mode', () => {
    ...
    describe('Error handling', () => {
      it('should handle role share error', async () => {
        const error = new Error('share fail');

        useInitialRoleSharing.mockReturnValue({ shareRole: jest.fn(() => Promise.reject(error)) });

        const { getByText } = renderComponent({ displayShareAction: true });

        await userEvent.click(getByText('stripes-authorization-components.shareToAll'));
        await userEvent.click(
          within(getByText('ui-consortia-settings.consortiumManager.modal.confirmShare.all.heading'))
            .getByRole('button', { name: 'confirm' })
        );
