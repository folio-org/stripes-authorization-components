...
describe('useCheckboxHandlers', () => {
  ...
  it('onChangeCapabilitySetCheckbox should disable previously saved capabilities when checked is FALSE ', () => {
    ...
    expect(setSelectedCapabilitySetsMap).toHaveBeenCalledWith({});
    expect(setDisabledCapabilities).toHaveBeenCalledWith({ 'cap15': true, 'cap16': true });
    expect(setSelectedCapabilitiesMap).toHaveBeenCalledWith({ 'cap1': true });
  });

  it('onChangeCapabilitySetCheckbox should remove initial capabilities on uncheck in edit flow', () => {
    const { onChangeCapabilitySetCheckbox } = getCheckboxHandlers({
      selectedCapabilitiesMap: { 'cap1': true, 'cap2': true },
      setSelectedCapabilitiesMap,
      capabilitySetsList: [
        {
          id: 'b7cbae7f-5d7a-4af5-bd4e-f70ad245ba2c',
      ...
      setSelectedCapabilitySetsMap,
      actionCapabilities,
      disabledCapabilities: { 'cap1': true, 'cap2': true, 'cap3': true },
      actionCapabilitySets,
      initialCapabilitiesSelectedMap: { 'cap1': true },
    });
    ...
    // cap1 is removed (it was in initialCapabilitiesSelectedMap), cap2 remains (session-selected)
    expect(setSelectedCapabilitiesMap).toHaveBeenCalledWith({ 'cap2': true });
  });
