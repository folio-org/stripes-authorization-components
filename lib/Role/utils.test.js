import {
  getCheckboxHandlers,
  getUnselectedCapabilitySetCapabilities,
  getUnselectedItemCounts,
  changesForUnselect,
} from './utils';
...
describe('useCheckboxHandlers', () => {
  ...
  describe('getUnselectedCapabilitySetCapabilities', () => {
    const expectUnselectedItems = (result, capabilitySets, capabilityItems) => {
      expect(result.unselectedCapabilitySets).toEqual(new Set(capabilitySets));
      expect(result.unselectedCapabilities).toEqual(new Set(capabilityItems));
    };

    it('returns unchecked capability set children for selected applications', () => {
      const capabilitySets = [
        { id: 'setA', resource: 'Set A', action: 'view', applicationId: 'app-1', capabilities: ['cap1_view', 'cap2_view'] },
      ];

      const result = getUnselectedCapabilitySetCapabilities(
        { 'app-1': true },
        capabilitySets,
        capabilities,
        ['setA'],
      ...
      expectUnselectedItems(result, ['Set A:view'], ['Cap 2:view']);
    });

    it('returns empty sets when unchecked capability sets are not in selected applications', () => {
      const capabilitySets = [
        { id: 'setA', resource: 'Set A', action: 'view', applicationId: 'app-1', capabilities: ['cap1_view'] },
      ];
      ...
      expectUnselectedItems(result, [], []);
    });

    it('ignores unchecked capability set ids that are not in the capability sets list', () => {
      const capabilitySets = [
        { id: 'setA', resource: 'Set A', action: 'view', applicationId: 'app-1', capabilities: ['cap1_view'] },
      ];
    ...
    it('returns capability sets even when all child capabilities are individually selected', () => {
      const capabilitySets = [
        { id: 'setA', resource: 'Set A', action: 'view', applicationId: 'app-1', capabilities: ['cap1_view', 'cap2_view'] },
      ];
      ...
      const result = getUnselectedCapabilitySetCapabilities(
        ...
        capabilities,
        ['setA'],
        { cap1_view: true, cap2_view: true },
      );

      expectUnselectedItems(result, ['Set A:view'], []);
    });

    it('aggregates multiple unchecked capability sets and de-duplicates child capabilities', () => {
      const capabilitySets = [
        { id: 'setA', resource: 'Set A', action: 'view', applicationId: 'app-1', capabilities: ['cap1_view', 'cap2_view'] },
        { id: 'setB', resource: 'Set B', action: 'manage', applicationId: 'app-2', capabilities: ['cap2_view', 'cap3_view'] },
      ];

      const result = getUnselectedCapabilitySetCapabilities(
        { 'app-1': true, 'app-2': true },
        capabilitySets,
        capabilities,
        ['setA', 'setB'],
        {},
      ...
      expectUnselectedItems(
        result,
        ['Set A:view', 'Set B:manage'],
        ['Cap 1:view', 'Cap 2:view', 'Cap 3:view'],
      );
    });
  });
