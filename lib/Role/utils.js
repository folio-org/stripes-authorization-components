 ...
 * @param {Object} params.initialCapabilitiesSelectedMap - Map of capabilities that were initially loaded from the API (used to distinguish loaded vs session-selected).
 *
...
export function getCheckboxHandlers({
  ...
  initialCapabilitiesSelectedMap = {},
}) {
  /**
  ...
  const onChangeCapabilitySetCheckbox = (event, capabilitySetId) => {
    ...
    if (checked) {
      ...
      const newSelectedCapabilitiesMap = { ...selectedCapabilitiesMap };
      capSetCapabilities.forEach((cap) => {
      ...
      setDisabledCapabilities(newDisabledCapabilities);
      setSelectedCapabilitiesMap(newSelectedCapabilitiesMap);
    }
    ...
    if (checked) {
      ...
      Object.keys(typeActionCapSets).forEach((capSet) => {
        ...
        found.capabilities.forEach((c) => {
          ...
          if (initialCapabilitiesSelectedMap[c]) {
            delete newSelectedCapabilitiesMap[c];
          }
        });
      });
