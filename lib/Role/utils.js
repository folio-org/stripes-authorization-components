 ...
 */
export function getUnselectedItemCounts(unselectedAppIds, items, checkedMap) {
  let unselectedCapabilitySetCount = 0;
  let unselectedCapabilityCount = 0;
    ...
    for (const item of itemList) {
      for (const versionedAppId of unselectedAppIds) {
        if (item.applicationId === getUnversionedAppId(versionedAppId) && checkedMap[item.id]) {
          // If item has capabilities property, it's a capability set and child capabilities should be counted as they are unselected too
          if (item.capabilities) {
            unselectedCapabilitySetCount++;
            unselectedCapabilityCount += item.capabilities.length;
          } else { // It's a capability
  ...
  return { unselectedCapabilitySetCount, unselectedCapabilityCount };
}
...
export function changesForUnselect(selectedAppIds, checkedAppIdsMap, capabilities, capabilitySets, selectedCapabilitiesMap, selectedCapabilitySetsMap) {
  ...
  if (unselectedAppIds.length) {
    const unselectedCapabilitySets = getUnselectedItemCounts(unselectedAppIds, capabilitySets, selectedCapabilitySetsMap);
    const unselectedCapabilities = getUnselectedItemCounts(unselectedAppIds, capabilities, selectedCapabilitiesMap);

    const unselectedCapabilitySetCount = unselectedCapabilitySets.unselectedCapabilitySetCount;
    const unselectedCapabilityCount = unselectedCapabilitySets.unselectedCapabilityCount + unselectedCapabilities.unselectedCapabilityCount;

    // If there are any capabilities or capability sets that would be unselected, show confirmation modal
    if (unselectedCapabilitySetCount > 0 || unselectedCapabilityCount > 0) {
      isConfirmationNeeded = true;
      return { isConfirmationNeeded, unselectedAppIds, unselectedCapabilitySetCount, unselectedCapabilityCount };
    }
  }
