...
/**
 * Gets the capability sets for a specific type and action,
 * returning an object where the keys are the capability set IDs and the values are true.
 *
 * @param {object} actionCapSets - Object containing capability sets categorized by type and action.
 * @param {string} type - the capability type.
 * @param {string} action - the capability action.
 * @returns {object} - An object with capability set IDs as keys and true as values.
 */
export function getTypeActionCapSets(actionCapSets, type, action) {
  return actionCapSets[type][action]?.reduce(
    (acc, value) => {
      acc[value] = true;
      return acc;
    },
...
export function getCheckboxHandlers({
  ...
  const toggleCapabilitySetsHeaderCheckbox = (checked, type, action) => {
    const newDisabledCapabilities = { ...disabledCapabilities };
    const typeActionCapSets = getTypeActionCapSets(actionCapabilitySets, type, action);
...
export function getUnselectedCapabilitySetCapabilities(selectedAppIds, capabilitySetsList, capabilities, unCheckedCapabilitySets, checkedCapabilities) {
        ...
        for (const action in cap.actions) {
          if (Object.hasOwn(cap.actions, action)) {
            ...
            foundCapSet.capabilities.forEach((subCapId) => {
              // If capabilities were previously selected by user, don't include them in count, since it will remain selected until user manually unselects them.
              if (cap.actions[action].includes(subCapId) && !checkedCapabilities[subCapId]) {
  ...
  return { unselectedCapabilitySets, unselectedCapabilities };
}
