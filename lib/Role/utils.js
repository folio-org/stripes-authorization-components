import { difference, find, toLength } from 'lodash';
...
export function getCheckboxHandlers({
   ...
   * @returns void
   */
  const onChangeCapabilitySetCheckbox = (checked, capabilitySetId) => {
   ...
   * @returns void
   */
  const toggleCapabilitiesHeaderCheckbox = (checked, type, action) => {
    if (!checked) {
   ...
   * @returns void
   */
  const toggleCapabilitySetsHeaderCheckbox = (checked, type, action) => {
    let { updatedCapabilitySetsMap, newSelectedCapabilities, newDisabledCapabilities } = getMappedCapabilities(selectedCapabilitySetsMap, selectedCapabilitiesMap, disabledCapabilities, actionCapabilitySets, type, action, checked, capabilitySetsList);
...
/**
 * Maps capabilities based on their selection state and associated capability sets.
 *
 * @param {Object} selectedCapabilitySetsMap - The map of selected capability sets.
 * @param {Object} selectedCapabilitiesMap - The map of selected capabilities.
 * @param {Object} disabledCapabilities - The map of disabled capabilities.
 * @param {Object} actionCapabilitySets - The map of action-specific capability sets.
 * @param {string} type - The type of the capability.
 * @param {string} action - The action of the capability.
 * @param {boolean} checked - The checked state of the checkbox.
 * @param {Array} capabilitySetsList - The list of capability sets.
 * @returns {Object} The updated maps and states.
 */
export const getMappedCapabilities = (selectedCapabilitySetsMap, selectedCapabilitiesMap, disabledCapabilities, actionCapabilitySets, type, action, checked, capabilitySetsList) => {
  let updatedCapabilitySetsMap = { ...selectedCapabilitySetsMap };
  let newSelectedCapabilities = { ...selectedCapabilitiesMap };
  let newDisabledCapabilities = { ...disabledCapabilities };
  ...
  if (checked) {
    ...
    Object.keys(typeActionCapSets).forEach((capSet) => {
      ...
      delete updatedCapabilitySetsMap[capSet];
  ...
  return { updatedCapabilitySetsMap, newSelectedCapabilities, newDisabledCapabilities };
}

/**
 * Gets the unselected capability set capabilities based on the selected applications.
 *
 * @param {Object} selectedAppIds - The map of selected application IDs.
 * @param {Array} capabilitySetsList - The list of capability sets.
 * @param {Object} capabilities - The map of capabilities.
 * @param {Array} unCheckedCapabilitySets - The list of unchecked capability sets.
 * @param {Object} checkedCapabilities - The map of checked capabilities.
 * @returns {Object} The unselected capability sets and capabilities.
 */
export function getUnselectedCapabilitySetCapabilities(selectedAppIds, capabilitySetsList, capabilities, unCheckedCapabilitySets, checkedCapabilities) {
  ...
  for (const checkedCapSetId of unCheckedCapabilitySets) {
    const foundCapSet = find(capabilitySetsList, (capSet) => {
      return capSet.id === checkedCapSetId && Object.keys(selectedAppIds).find((id) => id === capSet.applicationId);
    });
    if (foundCapSet) {
  ...
  return { unselectedCapabilitySets: unselectedCapabilitySets, unselectedCapabilities: unselectedCapabilities };
}
