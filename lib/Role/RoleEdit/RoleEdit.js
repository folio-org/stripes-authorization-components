...
export const RoleEdit = ({ path, tenantId }) => {
  ...
  const onChangeCapabilitySetCheckbox = (event, capabilitySetId) => {
    ...
    setDisabledCapabilities(prevDisabledCaps => ({ ...prevDisabledCaps, ...capabilitySetsCap }));
  ...
  const toggleCapabilitySetsHeaderCheckbox = (event, type, action) => {
    ...
    const newDisabledCaps = capabilitySets[type]
      .filter(capSet => capSet.id in typeActionCapSets)
    ...
    setDisabledCapabilities(prevDisabled => ({ ...prevDisabled, ...newDisabledCaps }));
  };
