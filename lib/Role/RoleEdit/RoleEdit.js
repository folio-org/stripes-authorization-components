...
import { getCheckboxHandlers, getUnselectedItemCount } from '../utils';
  ...
  const onSubmitSelectApplications = (appIds, onCloseHandler) => {
    ...
    if (unselectedAppIds.length) {
      ...
      if (unselectedCapabilityCount > 0 || unselectedCapabilitySetCount > 0) {
        ...
        setUnselectedItemsInfo({ selectedAppIds: appIds, unselectedAppIds, unselectedCapabilityCount, unselectedCapabilitySetCount, onCloseHandler });
        setIsUnselectApplicationConfirmationOpen(true);
      }
    }

    // Changes can be applied directly if no capabilities or capability sets would be unselected
    if (!isConfirmationNeeded && onCloseHandler) {
