...
import { isCapabilityVisible, isCapabilityDirect, isShared } from '../../utils';
...
export const RoleEdit = ({ path, tenantId }) => {
  ...
  // This useRoleCapabilities hook provides two main pieces of data for the view:
  // - initial state of selected capabilities for the role (all the checkboxes in the capabilities accordion.)
  // - a list of associated application ID's that is used to fetch associated capabilities and sets.
  const {
  ...
  // This useRoleCapabilitySets hook provides
  // - initial state of selected capability sets (checkboxes in the "Capability Sets" accordion)
  // - a list of associated application ID's that is used to fetch associated capabilities and sets.
  const {
  ...
  } = useRoleCapabilitySets(roleId, tenantId, {}, visibilityFilter);
  ...
  // Stable "assigned" state from the persisted roles' caps/capsets.
  const assignedAppIdsMap = useMemo(
    () => ({ ...capabilitiesAppIds, ...capabilitySetsAppIds }),
    [capabilitiesAppIds, capabilitySetsAppIds],
  );
  ...
  */
  ...
  const isInitialDataReady = isInitialRoleCapabilitySetsLoaded && isInitialRoleCapabilitiesLoaded
    && !isRoleCapabilitiesFetching && !isRoleCapabilitySetsFetching;
  ...
  useEffect(() => {
    if (isInitialDataReady) {
      ...
      // Checked apps in the choose applications modal. Also used to fetch possible capabilities/sets for the view.
      setCheckedAppIdsMap({ ...capabilitiesAppIds, ...capabilitySetsAppIds });
      // Directly selected capabilities (NOT inherited) checkboxes in the Capabilities accordion.
      setSelectedCapabilitiesMap({ ...initialRoleCapabilitiesSelectedMap });
      // Selected Capability Sets in the capability sets accordion.
      setSelectedCapabilitySetsMap({ ...initialRoleCapabilitySetsSelectedMap });
      // Inherited capabilities presented in the Capabilities accordion - presented as 'readonly' checkboxes.
  ...
  return (
    <RoleForm
      ...
      capabilitySetsList={capabilitySetsList}
      actionCapabilitySets={actionCapabilitySets}
      ...
      selectedCapabilitySetsMap={selectedCapabilitySetsMap}
