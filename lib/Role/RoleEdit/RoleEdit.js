...
export const RoleEdit = ({ path, tenantId }) => {
  ...
  // Selected capabilities filter - We always only need the direct, non-inherited capabilities for this and
  // we add the visibility filter based on state.

  const selectedCapabilitiesFilter = useMemo(
    () => composeFilters(visibilityFilter, isCapabilityDirect),
    [visibilityFilter],
  );
