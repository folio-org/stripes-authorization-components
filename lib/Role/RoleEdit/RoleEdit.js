...
export const RoleEdit = ({ path, tenantId }) => {
  ...
  const onSubmitSelectApplications = (appIds, onCloseHandler) => {
    ...
    if (unselectedAppIds.length) {
      for (const [key, capabilityList] of Object.entries(capabilities)) { //TODO: capabilitySets
        for (const capability of capabilityList) {

          for (const appId of unselectedAppIds) {
            if (capability.applicationId === getApplicationName(appId)) {
    ...
    if (unselectedCapabilityCount > 0) {
      showCallout({
        type: 'warning',
        messageId: 'stripes-authorization-components.applications.unselect.warning',
        values: { appNames: unselectedAppIds.join(', '), capabilityCount: unselectedCapabilityCount, capabilitySetCount: unselectedCapabilitySetCount },
      });
    }
