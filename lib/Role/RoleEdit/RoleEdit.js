...
export const RoleEdit = ({ path, tenantId }) => {
  ...
  const onSubmitSelectApplications = (appIds, onCloseHandler) => {
    ...
    const unselectedAppIds = difference(Object.keys(checkedAppIdsMap), Object.keys(appIds));

    if (unselectedAppIds.length) {
      const unselectedCapabilityCount = getUnselectedItemsCount(unselectedAppIds, capabilities, selectedCapabilitiesMap);
      const unselectedCapabilitySetCount = getUnselectedItemsCount(unselectedAppIds, capabilitySets, selectedCapabilitySetsMap);

      if (unselectedCapabilityCount > 0 || unselectedCapabilitySetCount > 0) {
        setUnselectedItemsInfo({ selectedAppIds: appIds, unselectedAppIds, unselectedCapabilityCount, unselectedCapabilitySetCount });
        setIsUnselectApplicationConfirmationOpen(true);
      }
    } else {
      applyAppIdsChanges(appIds);
    }
  };
  ...
  const getUnselectedItemsCount = (unselectedAppIds, items, checkedMap) => {
    let unselectedCapabilityCount = 0;

    for (const [key, capabilityList] of Object.entries(items)) {
      for (const capability of capabilityList) {

        for (const appId of unselectedAppIds) {
          if (capability.applicationId === getApplicationName(appId) && checkedMap[capability.id]) {
    ...
    return unselectedCapabilityCount;
  }
