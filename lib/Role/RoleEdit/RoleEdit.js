...
export const RoleEdit = ({ path, tenantId }) => {
  ...
  const {
    ...
    // expand=true returns all capabilities inherited from sets. The view needs this properly display
    // In order for the UI to properly reflect the initial state of assigned capabilities/sets/apps, all of
    // the assigned capabilities must be loaded.
    // If this expand=false is used, capabilities present in the detail view
    // will not be present in the role form. (UISAUTHCOM-96)
