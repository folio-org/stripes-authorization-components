...
import { getCheckboxHandlers, determineIfConfirmationNeededForUnselectedApps } from '../utils';
...
export const RoleEdit = ({ path, tenantId }) => {
  ...
  const onSubmitSelectApplications = (selectedAppIds, onCloseHandler) => {
    const unselectedAppInfo = determineIfConfirmationNeededForUnselectedApps(selectedAppIds, checkedAppIdsMap,
      capabilities, capabilitySets, selectedCapabilitiesMap, selectedCapabilitySetsMap);

    if (unselectedAppInfo.isConfirmationNeeded) {
      setUnselectedItemsInfo({ ...unselectedAppInfo, onCloseHandler });
