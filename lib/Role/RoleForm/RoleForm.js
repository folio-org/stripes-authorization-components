...
export const RoleForm = ({
  ...
  const hasInvalidNameCharacter = roleName?.includes('/');
  const isNameMissing = isNameBlurred && !roleName;
  const isNameInvalid = !roleName || hasInvalidNameCharacter;

  const nameError = isNameMissing
    ? <FormattedMessage id="stripes-core.label.missingRequiredField" />
    : hasInvalidNameCharacter && <FormattedMessage id="stripes-authorization-components.form.errors.name.invalidCharacter" />;
