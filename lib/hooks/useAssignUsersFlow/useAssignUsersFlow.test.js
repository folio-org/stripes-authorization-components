...
describe('useAssignUsersFlow', () => {
  ...
  it('calls onSubmit selected users', async () => {
    ...
    await act(async () => {
      await result.current.onSubmitSelectedUsers([{ id: '1', username: 'user1' }, { id: '3', username: 'user3' }]);
    });
    await act(async () => {
  ...
  it('aborts assignment and sends an error callout if a selected user has no username', async () => {
    getNonKeycloakUsers.mockClear();
    const assignRolesMock = jest.fn().mockResolvedValue();
    useAssignRolesToUserMutation.mockReturnValue({ mutateAssignRolesToUser: assignRolesMock });
    ...
    const { result } = renderHook(() => useAssignUsersFlow({
      roleId: '1',
      tenantId: 'diku2',
      users: [{ id: '2', username: 'existinguser' }],
      refetch: jest.fn()
    }), { wrapper });

    await act(async () => {
      await result.current.onSubmitSelectedUsers([{ id: '1', username: 'someuser' }, { id: '3' }]);
    });

    expect(sendCallout).toHaveBeenCalledWith(expect.objectContaining({
      type: 'error',
      timeout: 0,
      message: 'stripes-authorization-components.assignUnassign.error.username',
    }));
    expect(getNonKeycloakUsers).not.toHaveBeenCalled();
