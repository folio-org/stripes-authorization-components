...
export function useAssignUsersFlow({ roleId, tenantId, users, refetch }) {
  ...
  const onSubmitSelectedUsers = async (newSelectedUsers) => {
    ...
    // present error if any newSelectedUsers do not have a username.
    if (newSelectedUsers.some((u) => !u.username)) {
      sendErrorCallout(intl.formatMessage({ id: 'stripes-authorization-components.assignUnassign.error.username' }));
      return;
    }
  ...
  const onSubmitCreateKeycloakUsersConfirmation = async () => {
    ...
    if (recordsWithError.length) {
      reportCreateKeycloakUserErrors(recordsWithError,
