...
import { isCapabilityDirect } from '../../utils';
...
describe('useRoleCapabilities', () => {
  ...
  it('fetches role capabilities', async () => {
    ...
    expect(result.current.capabilitiesTotalCount).toEqual(3);
  ...
  it('applies the isCapabilityDirect filter to initialRoleCapabilitiesSelectedMap when passed in', async () => {
    const { result } = renderHook(() => useRoleCapabilities(1, '', false, {}, isCapabilityDirect), { wrapper });

    await act(() => result.current.isSuccess);

    expect(result.current.initialRoleCapabilitiesSelectedMap).toEqual(expectedInitialDirectCapabilitiesSelectedMap);
  });

  it('returns an empty initialRoleCapabilitiesSelectedMap when the isCapabilityDirect filter excludes every capability', async () => {
    ...
    expect(result.current.initialRoleCapabilitiesSelectedMap).toEqual({});
  });
});
