...
describe('useRoleCapabilities', () => {
  ...
  it('fetches role capabilities', async () => {
    const { result } = renderHook(() => useUserCapabilities(1), { wrapper });

    await act(() => result.current.isSuccess);

    expect(result.current.isSuccess).toBe(true);
    expect(result.current.initialUserCapabilitiesSelectedMap).toEqual(expectedInitialRoleCapabilitiesSelectedMap);
    expect(result.current.capabilitiesTotalCount).toEqual(2);
    expect(result.current.groupedUserCapabilitiesByType).toEqual(expectedGroupedRoleCapabilitiesByType);
