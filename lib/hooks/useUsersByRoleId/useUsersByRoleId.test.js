...
import { chunkedUsersReducer, useUsersByRoleId } from './useUsersByRoleId';
...
describe('useUsersByRoleId', () => {
  ...
  describe('should not fetch data', () => {
    let kyMock;
    beforeEach(() => {
      kyMock = jest.fn().mockReturnValue({
        json: jest.fn().mockResolvedValue({}),
      });
      useOkapiKy.mockReturnValue({ get: kyMock });
    });

    it('if `enabled` is false', async () => {
      renderHook(() => useUsersByRoleId('roleId', { enabled: false }), { wrapper });
      expect(kyMock).not.toHaveBeenCalled();
    });

    it('if `id` is not present', async () => {
      renderHook(() => useUsersByRoleId(), { wrapper });
      expect(kyMock).not.toHaveBeenCalled();
    });
  });
});
