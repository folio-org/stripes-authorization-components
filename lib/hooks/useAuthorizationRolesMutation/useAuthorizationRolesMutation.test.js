...
describe('useAuthorizationRolesMutation', () => {
  ...
  it('should not contain forward-slash characters in the duplicated role name', async () => {
    useIntl.mockReturnValueOnce({
      formatMessage: jest.fn((message) => message.id),
      formatDate: jest.fn(() => '08/25/2026, 10:00:00'),
    });
    ...
    const { name } = postMock.mock.calls[0][1].json;

    expect(name).not.toEqual(expect.stringContaining('/'));
  });
