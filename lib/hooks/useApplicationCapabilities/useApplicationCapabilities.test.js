...
describe('useApplicationCapabilities', () => {
  ...
  it('should apply the provided filter function to the capabilities', () => {
    const items = [
      { id: 1, applicationId: 'cap1', type: 'data', action: 'edit', resource: 'r1' },
      { id: 12, applicationId: 'cap12', type: 'data', action: 'create', resource: 'r1' },
    ];
    ...
    const filter = (capability) => capability.applicationId === 'cap1';
    ...
    expect(result.current.actionCapabilities).toStrictEqual({
      data: { edit: [1] },
      procedural: {},
      settings: {},
    });
  ...
  it('should retain checked state and keep selectedCapabilitiesMap unfiltered when the visibility filter is toggled', async () => {
    const items = [
      { id: 'cap-visible', applicationId: 'cap1', type: 'data', action: 'view', resource: 'r1', visible: true },
      { id: 'cap-hidden', applicationId: 'cap1', type: 'data', action: 'view', resource: 'r2', visible: false },
    ];
    useChunkedApplicationCapabilities.mockClear().mockReturnValue({ items, isLoading: false });

    const showOnlyVisible = (capability) => capability.visible;
    const showAll = () => true;
    ...
    // Only the visible capability is present in the filtered, displayed data.
    expect(result.current.capabilities.data.map(({ id }) => id)).toEqual(['cap-visible']);

    // Simulate the role already having a checked capability that happens to be hidden.
    await waitFor(() => {
      result.current.setSelectedCapabilitiesMap({ 'cap-visible': true, 'cap-hidden': true });
    });

    // Toggle "show hidden" on: the filter changes, revealing the hidden capability...
    rerender({ checkedAppIdsMap: { cap1: true }, setDisabledCapabilities: jest.fn(), filter: showAll });

    expect(result.current.capabilities.data.map(({ id }) => id)).toEqual(['cap-visible', 'cap-hidden']);
    // ...but the checked state was never filtered in the first place, so it is unchanged.
    expect(result.current.selectedCapabilitiesMap).toEqual({ 'cap-visible': true, 'cap-hidden': true });

    // Toggle "show hidden" back off: the displayed list is filtered again...
    rerender({ checkedAppIdsMap: { cap1: true }, setDisabledCapabilities: jest.fn(), filter: showOnlyVisible });

    expect(result.current.capabilities.data.map(({ id }) => id)).toEqual(['cap-visible']);
    // ...but the checked state for the now-hidden capability is still retained.
    expect(result.current.selectedCapabilitiesMap).toEqual({ 'cap-visible': true, 'cap-hidden': true });
  });
});
