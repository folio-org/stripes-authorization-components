...
export const useRoleCapabilitySets = (roleId, tenant = '', options = {}, filter = (_capabilitySet) => true) => {
  ...
  */
  ...
  return {
    initialRoleCapabilitySetsSelectedMap, // state value, unfiltered
    initialRoleCapabilitySetsNames, // unfiltered.
    isSuccess,
    isLoading,
    isFetching,
    capabilitySetsTotalCount: filteredCapabilitySets?.length || 0,
    groupedRoleCapabilitySetsByType, // filtered by the provided filter.
    capabilitySetsCapabilities, // state value, unfiltered
    capabilitySetsAppIds, //state value, unfiltered
  };
};
