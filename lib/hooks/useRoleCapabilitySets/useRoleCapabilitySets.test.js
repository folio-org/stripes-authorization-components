...
describe('useRoleCapabilitySets', () => {
  ...
  it('applies the provided filter function to the fetched capability sets', async () => {
    const filter = (capabilitySet) => capabilitySet.type === 'data';

    const { result } = renderHook(() => useRoleCapabilitySets(1, '', {}, filter), { wrapper });

    await act(() => result.current.isSuccess);

    expect(result.current.isSuccess).toBe(true);
    expect(result.current.capabilitySetsTotalCount).toEqual(1);
    expect(result.current.initialRoleCapabilitySetsSelectedMap).toStrictEqual({
      'data-capability-id': true,
    });
    expect(result.current.groupedRoleCapabilitySetsByType).toEqual({
      data: expectedGroupedRoleCapabilitiesByType.data,
      procedural: [],
      settings: [],
    });

    // filter is deliberately not applied to these fields
    expect(result.current.capabilitySetsAppIds).toEqual({ 'app-platform-minimal-0.0.4': true });
    expect(result.current.capabilitySetsCapabilities).toBeDefined();
  });
});
