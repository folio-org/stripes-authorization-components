 ...
 *    Unfiltered fields:
 *      These are typically used to set/derive state and other bookkeeping logic.
 *      - `capabilitySets`
...
export const useApplicationCapabilitySets = ({ checkedAppIdsMap, options = {}, filter = defaultFilter }) => {
  ...
  const roleCapabilitySetsListNames = useMemo(() => {
    ...
    return roleCapabilitySetsListIds
      ...
      .filter(Boolean);
  }, [roleCapabilitySetsListIds, capabilitySets]);
  ...
  // The form state should not be filtered. The selected state is independent of the filtered view.
